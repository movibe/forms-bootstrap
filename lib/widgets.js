...
exports.checkbox = function (opt) {
    ...
    var w = {
     ...
     w.toHTML = function (name, f) {
        f = f || {};
        /*Hidden false checkbox to handle unchecked case*/
        var html = '<input type="hidden" value="false" ';
        html += attrs({name: name, id: f.id, classes: w.classes});
        html += ' />';
        /*Actual Checkbox*/
...
exports.select = function (opt) {
    ...
    w.toHTML = function (name, f) {
        ...
        html += Object.keys(f.choices).reduce(function (html, k) {
            return html + '<option value="' + k + '"' + ((f.value && (f.value === k || f.value == k)) ? ' selected="selected"' : '') + '>' + f.choices[k] + '</option>';
        }, '');
        return html + '</select>';
    };
    return w;
    ...
    w.toHTML = function (name, f) {
        ...
        return Object.keys(f.choices).reduce(function (html, k) {
            ...
            if (Array.isArray(f.value)) {
                if (f.value.some(function (v) { return (v === k || v == k); })) {
                    html += ' checked="checked"';
                }
            } else {
                html += (f.value === k || f.value == k) ? ' checked="checked"' : '';
            }
...
exports.multipleSelect = function (opt) {
    ...
    w.toHTML = function (name, f) {
        ...
        html += Object.keys(f.choices).reduce(function (html, k) {
            ...
            } else if (f.value && (f.value === k || f.value == k)) {
                html += ' selected="selected"';
            }
